...
1. **Map this plan to STARNET** using the **## STARNET catalog** table in the same context pack when it has rows. Use **exact** holon/OAPP names and **catalog ids** from that table. If you need fields beyond the table, use **mcp_invoke** with \`star_get_holon\` / \`star_get_oapp\` on those ids (Agent mode only).
